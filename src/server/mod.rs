...
impl FileServer {
   pub fn new(address: &str, port: &str, thread_count: i32, root_dir:&'static str) -> Result<FileServer, FileServerError> {
    ...
    fn determine_handler(&self,mut stream: &TcpStream) -> Result< fn(stream:&TcpStream,root_dir:&'static str),FileServerError>{
    ...
    pub fn register_handlers(&mut self,handlers: &[(CommandType,fn(stream:&TcpStream,root_dir:&'static str))]) {
...
mod tests {
    ...
    fn test_download_file() {
        ...
        let server = setup_file_server(addr, port, 3, &[(
